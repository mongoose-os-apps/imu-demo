    ...
    for (i = 0; i < inBytes.length; i += 1) {
        ...
        if ((inBytes[i] === 0x0d) || (inBytes[i] === 0x0a)) {
            ...
            if (myString.length>0) {
              if (myString.startsWith("$P>1:")) {
                imu_packet_info(myString.slice(5));
              } else if (myString.startsWith("$P>2:")) {
                imu_packet_stats(myString.slice(5));
              } else if (myString.startsWith("$P>3:")) {
                imu_packet_data(myString.slice(5));
              } else {
                console.log("Unknown Input:", myString);
              }
            }
            myString = "";
            lineStart = lineEnd + 1;
